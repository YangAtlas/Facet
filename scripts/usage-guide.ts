...
export async function usageGuide(){
 ...
 list(['点击右侧组件卡片，在当前光标处插入。收藏、最近使用和分类标签帮助你快速定位。','在空白段落输入 /，键入名称或英文别名，用方向键选择后按 Enter；Esc 关闭菜单。','使用 ⌘⇧P 或“书写命令”搜索组件；工具栏可上移、下移、复制和删除当前组件。','桌面版按 ⌘S 保存 .facet；它同时保存文字、结构和本地图片。恢复草稿可找回上一次编辑。']),
 p('按 Ctrl+F / ⌘F 搜索全文，Enter 查找下一处，Shift+Enter 查找上一处，Esc 关闭搜索。输入 $ 自动补齐一对，再输入 $ 成为两对，光标保持在中间；括号、中文括号及引号也支持配对。'),
 h('文档标题、目录、页眉和页面设置'),
 p('左下角设置包含封面、目录、导出、收藏、说明书和通用选项。收藏按基础、提示框、多媒体和页面分类，支持搜索；勾选的样式会进入右侧收藏夹；提示框包含信息、建议、定义、实验、结论等全部样式。'),
 p('链接可以输入 example.com、www 开头的网址、邮箱或带协议的地址；按住 Ctrl / ⌘ 时出现打开链接提示。双击引用块可选择颜色。表格内相邻列之间的边界可左右拖动，调整后的宽度会随文档保存。'),
 p('正文右键提供保留格式、匹配当前样式、纯文本、Markdown 和图片粘贴。Ctrl+Shift+V / ⌘⇧V 粘贴纯文本。PPT、Excel 等来源的表格优先作为可编辑表格插入，按当前页面宽度保留列比例；合并单元格和空单元格一并保留。仅有图片数据时可粘贴为图片。左侧文件管理可打开最近文档、查看位置、另存文档和恢复本机草稿。编辑时自动保留恢复草稿；按 Ctrl+S / ⌘S 写入原文件。切换文件或关闭窗口时，未保存更改会提示保存、不保存或取消。左侧文件树支持打开本地文件夹、展开子文件夹与打开文档，顶部显示当前文件夹名称。'),
 p('打开左下角“设置 → 封面”编辑文档标题、副标题、作者、机构、日期与封面图片。本手册首页就是文档标题组件；下一页的目录自动汇总标题和页码，点击条目可跳转。'),
 p('页眉居中显示。新建文档可选 A4 竖版或 16:9 横版，“设置 → 封面”还可调整页边距、页脚与主题色。纸面 PDF 保留当前分页；Beamer PDF 会重新按标题组织幻灯片。'),
 card('info','本手册目录展示 Part、一级和二级标题；三级标题仍作为 Beamer 的分帧边界。设置中的目录可选择显示到一级、二级或三级标题；双击标题可以调整编号及图标。'),
 h('部分与三级标题'),
 ...
 b('part',{},[text('Part 2 · 提示框')]),h('让观点、证据与边界各有位置',2),
 h('提示框：信息、建议与注意'),
 p('点击对应卡片或输入英文命令即可插入。标题和图标可点击编辑；双击提示框可以调整颜色、自定义标题和图标。'),
 ...
 h('提示框：文献记录'),card('literature','/literature 文献：记录作者、年份、题名、来源、关键结论及与当前研究的关系。本例为用法示范，不虚构参考文献。'),
 p('卡片也可以包含列表、待办或公式。长卡片可以续页，但特别长的单个公式仍应手动分行。'),
 b('part',{},[text('Part 3 · 公式、代码与多媒体')]),h('将证据放入文档',2),
 h('公式：块公式与行内公式'),
 p('输入 $x^2$ 可生成行内公式；输入 $$x+y$$ 可生成独立公式，也可以输入 $$ 后按 Enter 打开公式面板。右键选择粘贴为 Markdown，支持多行公式。'),
 ...
 h('代码：CodeSnap 风格'),
 p('使用 /code 插入代码块，右上角选择语言，在代码区直接输入。代码跨页时行号连续，三色圆点、深色背景、行号与语法高亮会一起进入 PDF。'),
 code('# 演示：将观测值汇总为均值\ndef summarize(values):\n    total = sum(values)\n    count = len(values)\n    return total / count if count else 0\n\nsamples = [0.82, 0.86, 0.88]\nprint(summarize(samples))'),
 h('图片：尺寸与题注'),p('用 /img 选择本地图片，也可以粘贴截图。双击图片调整宽度和题注。此处使用本地生成的示意图。'),image(),
 ...
 h('表格：实线网格与主题色'),
 p('插入时选择“实线网格 · 主题配色”，或在双击已有表格后切换。网格表适合任务安排和清单。'),
 table([['组件','使用场景','入口'],['待办清单','行动跟踪','/todo'],['状态标签','进展标记','/state'],['图标列表','语义提示','/iconlist']],'grid'),
 b('part',{},[text('Part 4 · 导出与验收')]),h('用这份手册验证导出',2),
 h('三种导出方式'),p('点击右上角导出按钮，选择纸面 PDF、Beamer PDF 或 LaTeX 源码包。左下角设置中的导出页保存默认格式与 Beamer 选项。'),list(['PDF：按当前纸面分页导出，保留页眉、页脚和组件样式。','Beamer PDF：按三级标题重新分帧，Part 生成分节页；过长内容自动生成“（续）”标题。','LaTeX 源码包：导出独立排版的源码和资源，使用 XeLaTeX 编译；分页和组件外观不承诺与纸面 PDF 完全相同。']),

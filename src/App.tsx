...
async function browserRecovery(bytes?:Uint8Array):Promise<Uint8Array|null>{
  return new Promise((resolve,reject)=>{const req=indexedDB.open('facet-local',1);req.onupgradeneeded=()=>req.result.createObjectStore('drafts');req.onerror=()=>reject(req.error);req.onsuccess=()=>{const db=req.result;const tx=db.transaction('drafts',bytes?'readwrite':'readonly');const action=bytes?tx.objectStore('drafts').put(bytes,'latest'):tx.objectStore('drafts').get('latest');action.onsuccess=()=>{if(!bytes)resolve(action.result??null)};tx.oncomplete=()=>{db.close();if(bytes)resolve(bytes)};tx.onerror=()=>reject(tx.error)}})
}
...
export default function App(){
  ...
  const [left,setLeft]=useState(preferences.left),[right,setRight]=useState(preferences.right),[query,setQuery]=useState(''),[group,setGroup]=useState('常用')
  ...
  const [settings,setSettings]=useState(false),[exportMenu,setExportMenu]=useState(false)
  ...
  const token=useRef<string|undefined>(undefined),savedRevision=useRef(-1),saveTimer=useRef<ReturnType<typeof setTimeout>|undefined>(undefined),saveQueue=useRef(Promise.resolve()),layoutTimer=useRef<ReturnType<typeof setTimeout>|undefined>(undefined)
  ...
  const editor=useEditor({extensions:ext,content:{type:'doc',content:[{type:'page',content:hydrate(initial)}]},editorProps:{
    ...
    handleKeyDown(view,event){
      ...
      if(s){const matches=searchComponents(s.q);if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();setSlash({...s,selected:Math.max(0,Math.min(matches.length-1,s.selected+(event.key==='ArrowDown'?1:-1)))});return true}if(event.key==='Escape'){setSlash(null);return true}if(event.key==='Enter'&&matches[s.selected]){event.preventDefault();view.dispatch(view.state.tr.delete(s.from,s.to));setSlash(null);inserting.current(matches[s.selected]);return true}}
      if((event.metaKey||event.ctrlKey)&&event.key==='s'){event.preventDefault();void flushRef.current();return true}
      return false
    },
    handlePaste(_view,event){const image=Array.from(event.clipboardData?.files??[]).find(f=>/^image\/(png|jpeg|webp)$/.test(f.type));if(image){event.preventDefault();void importImages([image]);return true}return false},
    handleDOMEvents:{compositionend(){setTimeout(()=>reflowRef.current(),0);return false}},
  ...
  const updateRecent=()=>{void window.facetDesktop?.recent().then(setRecentFiles)}
  const enqueueSave=async(manual=false,saveAs=false)=>{
    const job=async()=>{
      const snapshot=structuredClone(docRef.current),bytes=await encodeDocument(snapshot)
      if(window.facetDesktop){
        const recovery=await window.facetDesktop.recovery({id:snapshot.id,bytes:Array.from(bytes)});if(recovery?.error)throw new Error(recovery.error)
        if(token.current||manual){
          const result=await window.facetDesktop.save({token:token.current,name:exportName||snapshot.metadata.title,bytes:Array.from(bytes),saveAs})
          if(result?.error)throw new Error(result.error)
          if(result?.conflict){setStatus('文件冲突 · 请另存');announce('文件已被其他应用修改。恢复副本已保留，请使用“另存为”。');return}
          if(!result){setStatus('草稿已保存在本机');return}
          token.current=result.token;savedRevision.current=snapshot.revision;setStatus(docRef.current.revision===snapshot.revision?'已保存':'正在保存…');updateRecent()
        }else{savedRevision.current=snapshot.revision;setStatus('草稿已保存在本机')}
      }else{await browserRecovery(bytes);savedRevision.current=snapshot.revision;setStatus('草稿已保存在本机');if(manual)download(bytes,`${snapshot.metadata.title}.facet`,'application/zip')}
    }
    const promise=saveQueue.current.then(job);saveQueue.current=promise.catch(e=>{setStatus('保存失败');announce(String(e.message??e))});return promise
  }
  const scheduleSave=()=>{clearTimeout(saveTimer.current);setStatus('正在保存…');saveTimer.current=setTimeout(()=>void enqueueSave().catch(()=>{}),1000)}
  ...
  useEffect(()=>{updateRecent();if(window.facetDesktop){void window.facetDesktop.recoveries().then(setRecoveries)}else{void browserRecovery().then(x=>{if(x)setRecoveries([{id:'browser',modified:Date.now()}])})}},[])
  useEffect(()=>{if(!window.facetDesktop)return;return window.facetDesktop.onClose(()=>{clearTimeout(saveTimer.current);void enqueueSave().then(()=>{if(status==='文件冲突 · 请另存')throw new Error('请先处理文件冲突');window.facetDesktop!.closeReady()}).catch(()=>{window.facetDesktop!.cancelClose();announce('保存未完成，窗口已保留，请先保存或另存文档。')})})})
  useEffect(()=>{if(!window.facetDesktop)return;return window.facetDesktop.onOpen(data=>void openData(data))})
  useEffect(()=>{const handle=(event:KeyboardEvent)=>{if(event.key==='Escape'){setAppSettings(false);setExportSettings(null);setNewDocumentOpen(false);setLinkDraft(null);setMath(null);setSettings(false);setEditNode(null);setExportMenu(false);setSlash(null);setCommandOpen(false);setComponentPicker(null);setTableSize(null);setImageModal(false)}if((event.ctrlKey||event.metaKey)&&event.shiftKey&&event.key.toLowerCase()==='p'){event.preventDefault();setQuery('');setCommandOpen(true)}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='n'){event.preventDefault();setNewOrientation(preferences.orientation);setNewDocumentOpen(true)}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='s'){event.preventDefault();void flushRef.current()}};window.addEventListener('keydown',handle);return()=>window.removeEventListener('keydown',handle)},[preferences.orientation])
  useEffect(()=>{if(!editor)return;const listener=(e:MouseEvent)=>{
    const target=e.target as HTMLElement
    if(target.closest('.document-title')){setSettings(true);return}
    const el=target.closest('[data-math],.callout-title,.badge,[data-inline-icon],.image-block,.subfigure,.semantic-list>li>i') as HTMLElement|null
    if(!el)return
    const parent=el.closest('[data-id]') as HTMLElement|null
    let found:{pos:number;node:any}|undefined
    editor.state.doc.descendants((node,pos)=>{if(found)return false;const dom=editor.view.nodeDOM(pos);if(dom instanceof HTMLElement && (dom===el||dom.contains(el)) && ['mathBlock','mathInline','callout','badge','inlineIcon','image','subfigure','semanticItem'].includes(node.type.name)){found={pos,node};return false}})
  ...
  const single=(e:MouseEvent)=>{const external=(e.target as HTMLElement).closest('a[href]') as HTMLAnchorElement|null;if(external&&(e.metaKey||e.ctrlKey)&&/^(https?:|mailto:)/i.test(external.href)){e.preventDefault();if(window.facetDesktop)void window.facetDesktop.openExternal(external.href);else window.open(external.href,'_blank','noopener,noreferrer');return}const link=(e.target as HTMLElement).closest('[data-toc-target]') as HTMLElement|null;if(link){e.preventDefault();const id=link.dataset.tocTarget;let pos:number|undefined;editor.state.doc.descendants((n,p)=>{if(['heading','part'].includes(n.type.name)&&n.attrs.id===id&&pos===undefined)pos=p});if(pos!==undefined){editor.commands.setTextSelection(pos+1);editor.view.focus();(editor.view.nodeDOM(pos) as HTMLElement)?.scrollIntoView({block:'start',behavior:'smooth'})}return}if((e.target as HTMLElement).closest('[data-math],.callout-title,.document-title,.semantic-list>li>i'))listener(e)}
  ...
  async function openData(data:any){if(!data)return;if(data.error){announce(data.error);return}try{const next=await decodeDocument(new Uint8Array(data.bytes));await enqueueSave();clearTimeout(saveTimer.current);token.current=data.token;savedRevision.current=next.revision;syncDoc(next);lastSelection.current=null;resetEditorDocument(next);context.current.outline=[];paginate();setStatus(data.token?'已保存':'已恢复草稿');updateRecent();setRecoveries([])}catch(e){announce(String((e as Error).message))}}
  async function newDocument(orientation:PageOrientation){if(creatingDocument)return;setCreatingDocument(true);try{await enqueueSave();clearTimeout(saveTimer.current);const next=makeDocument(false,orientation);token.current=undefined;syncDoc(next);lastSelection.current=null;context.current.outline=[];resetEditorDocument(next);paginate();scheduleSave();setNewDocumentOpen(false);setSettings(true)}catch(e){announce(String(e))}finally{setCreatingDocument(false)}}
  ...
  async function recover(id:string){const data=window.facetDesktop?await window.facetDesktop.recover(id):{bytes:Array.from(await browserRecovery()??[])};await openData(data)}
  ...
  const common=['info','tip','important','reflection','equation','image','table','code','heading1','task']
  const visible=query?matches:group==='常用'?common.map(id=>registry.find(x=>x.id===id)!):group==='最近使用'?recentComponents.map(id=>registry.find(x=>x.id===id)!).filter(Boolean):matches.filter(x=>x.group===group)
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">◈</span><span>Facet</span><span className="brand-separator"/><span className="workspace-label">个人工作空间</span></div><div className="document-status"><span>{doc.metadata.title}</span><span className="status-dot"/><small>{status}</small></div><div className="top-actions"><button className="text-button" onClick={()=>setAppSettings(true)}><Settings2 size={16}/>应用设置</button><button className="text-button" onClick={()=>setSettings(true)}><Settings2 size={16}/>页面设置</button><div className="export-wrap"><button className="primary-button" disabled={busy} onClick={()=>setExportMenu(!exportMenu)}><Download size={15}/>{busy?'正在导出…':'导出'}<ChevronDown size={13}/></button>{exportMenu&&<div className="dropdown export-dropdown"><button onClick={()=>configureExport('pdf')}>导出 PDF <small>与纸面逐页一致</small></button><button onClick={()=>configureExport('beamer')}>Beamer PDF <small>三级标题分组 · 自动续页</small></button><button onClick={()=>configureExport('latex')}>LaTeX 源码包 <small>独立排版版本</small></button><hr/><button onClick={()=>void enqueueSave(true)}>保存 .facet 文档</button><button onClick={()=>void enqueueSave(true,true)}>另存为…</button></div>}</div></div></header>
    <div className="main-layout">
      {left&&<aside className="left-panel"><div className="panel-heading"><span>文档</span><button className="icon-button" title="收起导航" onClick={()=>setLeft(false)}><PanelLeftClose size={17}/></button></div><button className="new-document" onClick={()=>{setNewOrientation(preferences.orientation);setNewDocumentOpen(true)}}><Plus size={16}/>新建文档<span>⌘ N</span></button><button className="nav-link" onClick={()=>void openDocument()}><FolderOpen size={16}/>打开文档</button><div className="nav-section-label">正在书写</div><button className="document-nav active"><FileText size={16}/><span>{doc.metadata.title}</span><span className="tiny-dot"/></button>{recentFiles.length>0&&<><div className="nav-section-label">最近文档</div>{recentFiles.slice(0,5).map(f=><button key={f.index} className="document-nav" onClick={()=>void openDocument(f.index)} title={f.directory}><FileText size={15}/><span>{f.name.replace(/\.facet$/,'')}</span></button>)}</>}
      ...
      {right&&<aside className="right-panel"><div className="panel-heading"><span>组件工具箱</span><button className="icon-button" title="收起组件" onClick={()=>setRight(false)}><PanelRightClose size={17}/></button></div><p className="panel-description">为想法找到合适的表达。</p><label className="component-search"><Search size={15}/><input placeholder="搜索组件…" value={query} onChange={e=>setQuery(e.target.value)}/><kbd>/</kbd></label><div className="component-tabs">{['常用','最近使用','基础','提示框','科研','多媒体','页面'].map(g=><button key={g} className={g===group?'active':''} onClick={()=>{setGroup(g);setQuery('')}}>{g}</button>)}</div><div className="component-list"><div className="component-section-heading">{query?'搜索结果':group}<span>{visible.length}</span></div>{visible.map(item=><button key={item.id} data-component={item.id} className={`component-card ${item.variant?'has-swatch':''}`} onMouseDown={e=>e.preventDefault()} onClick={()=>insertComponent(item)}><span className="component-icon" style={item.variant?{'--component-color':colors[variants[item.variant].color]} as React.CSSProperties:{}}>{fa(item.icon)}</span><span className="component-copy"><strong>{item.label}</strong><small>{item.description}</small>{item.syntax&&<code className="component-syntax">{item.syntax}</code>}</span><Plus className="component-plus" size={14}/>{item.variant&&<span className="component-swatch" style={{background:colors[variants[item.variant].color]}}/>}</button>)}{!visible.length&&<p className="muted-empty">{group==='最近使用'?'使用过的组件会出现在这里。':'没有找到匹配的组件。'}</p>}</div><div className="quickadd-note"><kbd>/</kbd><div><strong>也可以用键盘，更快一点</strong><span>输入 / 搜索组件，按 Enter 插入</span></div></div></aside>}
    </div>
    ...
    {editNode&&<Modal title="组件设置" onClose={()=>setEditNode(null)}><div className="settings-grid">{editNode.type==='callout'&&<><label>卡片类型<select value={editNode.attrs.variant} onChange={e=>{const v=variants[e.target.value];setEditNode({...editNode,attrs:{...editNode.attrs,variant:e.target.value,...v}})}}>{Object.entries(variants).map(([k,v])=><option value={k} key={k}>{v.title||'重点段落'}</option>)}</select></label><label>标题<input value={editNode.attrs.title??''} onChange={e=>setEditNode({...editNode,attrs:{...editNode.attrs,title:e.target.value}})}/></label></>}
      ...
      {editNode.type==='badge'&&<label>标签文字<input value={editNode.attrs.text} onChange={e=>setEditNode({...editNode,attrs:{...editNode.attrs,text:e.target.value}})}/></label>}
      ...
      {!['callout','badge','inlineIcon','heading','image','subfigure','columns','codeBlock','semanticItem'].includes(editNode.type)&&<p className="muted-empty">此组件可以直接在纸面上编辑。</p>}</div><div className="modal-actions"><button className="secondary-button" onClick={()=>setEditNode(null)}>取消</button><button className="primary-button" onClick={()=>{if(editor){editor.view.dispatch(editor.state.tr.setNodeMarkup(editNode.pos,undefined,editNode.attrs));editor.view.focus()}setEditNode(null)}}>应用</button></div></Modal>}

...
test.beforeEach(async({page})=>{await page.goto('/');await page.waitForFunction(()=>window.__facet?.editor)})
test('every component advertises a working slash alias',async({page})=>{
  const missing=await page.evaluate(async()=>{const path='/src/core/registry.ts';const {registry,searchComponents}=await import(/* @vite-ignore */ path);return registry.filter((c:any)=>!c.syntax.includes('/'+c.aliases[0])||!searchComponents(c.aliases[0]).some((m:any)=>m.id===c.id)).map((c:any)=>c.id)})
...
test('app preferences persist and all export formats open settings first',async({page})=>{
  await page.getByRole('button',{name:'应用设置',exact:true}).click()
  await page.getByLabel('默认页面方向').selectOption('landscape');await page.getByLabel('显示组件工具箱').uncheck();await page.getByRole('button',{name:'完成',exact:true}).click()
  ...
  for(const label of ['导出 PDF','Beamer PDF','LaTeX 源码包']){
    await page.getByRole('button',{name:'导出',exact:true}).click();await page.locator('.export-dropdown').getByRole('button',{name:new RegExp(label)}).click()
